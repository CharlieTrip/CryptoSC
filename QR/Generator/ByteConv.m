...
if version < 10
ArrayByte = cat(2,[0,1,0,0],de2bi(length(Stringa),8,'left-msb'));
elseif and(version >= 10, version < 27)
 ArrayByte = cat(2,[0,1,0,0],de2bi(length(Stringa),16,'left-msb'));
else
 ArrayByte = cat(2,[0,1,0,0],de2bi(length(Stringa),16,'left-msb'));
end


%appendo i byte corrispondentia ciascun carattere della stringa
...
%% Non funziona, però se si fa passo a passo funziona problema con l'input
